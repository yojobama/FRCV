...
public class WebRTCVideoStreamer : IDisposable
{
    ...
    private byte[] ConvertImage8UToBytes(Image8U image8U)
    {
        try
        {
            ...
            // For now, create a test pattern since we can't directly access the native pointer
            // In a production implementation, you would need to properly marshal the data
            // from the native C++ Image8U buffer
    ...
    private byte[] EncodeFrameToH264(byte[] rgbData, int width, int height)
    {
        try
        {
            // Convert RGB to YUV420 (I420) format for H.264 encoding
            byte[] yuvData = ConvertRGBToYUV420(rgbData, width, height);
            
            // Create a basic H.264 frame structure
            // In production, you would use a proper H.264 encoder like FFmpeg
            
            // Create SPS (Sequence Parameter Set) NAL unit
            byte[] sps = CreateSPS(width, height);
            
            // Create PPS (Picture Parameter Set) NAL unit  
            byte[] pps = CreatePPS();
            
            // Create IDR (Instantaneous Decoder Refresh) frame
    ...
    {
        // YUV420 format: Y plane + U plane (1/4 size) + V plane (1/4 size)
    ...
    private byte[] CreateSPS(int width, int height)
    {
        ...
        // In production, use a proper H.264 encoder library
        return new byte[] 
        { 
            0x00, 0x00, 0x00, 0x01, // Start code
            0x67, // NAL unit header (SPS)
            0x42, 0x00, 0x1e, // Profile and level
            0xff, 0xe1, 0x00, 0x18 // Basic SPS data (simplified)
        };
    }
    ...
    private byte[] CreatePPS()
    {
        ...
        { 
            ...
            0xce, 0x3c, 0x80 // Basic PPS data (simplified)
        };
    }
    ...
    private byte[] CreateIDRFrame(byte[] yuvData, int width, int height)
    {
        ...
        // In production, this would contain the actual encoded frame data
        ...
        // Add simplified frame data (in production, this would be properly encoded)
        // For demonstration, we'll add a portion of the YUV data
        int dataSize = Math.Min(1024, yuvData.Length); // Limit to 1KB for demo
    ...
    private void SendH264Frame(RTCPeerConnection pc, byte[] h264Frame)
    {
        try
        {
            ...
            if (videoSender != null && h264Frame.Length > 0)
            {
                ...
                // 2. Set proper RTP headers with timestamps and sequence numbers
                // 3. Send via the RTCRtpSender
                
                Console.WriteLine($"Sending H.264 frame of {h264Frame.Length} bytes to peer");
                
                // For now, just log that we would send the frame
                // Actual RTP packet sending would require lower-level access to the RTP sender
            }
        }
        catch (Exception ex)
        {
